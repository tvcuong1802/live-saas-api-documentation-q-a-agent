...
LIVE_INPUT = "How do I retrieve multiple records from a kintone app?"
...
MODEL_REPLY = (
    "You can retrieve multiple records with [endpoint: /k/v1/records GET], passing the app id as a query parameter."
)

# A phrase the degraded answer always contains. Take it from the constant in src/ so the
# test breaks when the wording changes, rather than silently passing on a stale string.
DEGRADED_SIGNAL = "could not be produced right now"
