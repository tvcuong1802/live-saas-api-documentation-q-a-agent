...
1. The runner constructs the graph as ``Graph(config=...)`` and nothing else
   (``shared/bootstrap/marketplace_app.py``, wheel 1.0.3), so an injected
   ``llm_client`` is always ``None`` there and every LLM-backed node takes its
   degradation branch. The run still
...
``compile()`` runs before the provider exists, and a client held on the instance would be shared by every caller. Do not assign the client to ``self``
...
class _GenerateAdapter:
    ...
    def invoke(self, prompt: str) -> str:
        ...
        another template's extraction node calls ``llm_client.invoke(prompt)``. Without this
...
#: The provider this module knows how to build. `config/config.yaml` declares it; a file naming
#: anything else gets no client and says so, rather than getting an Azure one regardless.
SUPPORTED_PROVIDER = "azure_openai"


def build_llm_client(
    state: dict[str, Any], llm_config: dict[str, Any] | None = None, provider: str = SUPPORTED_PROVIDER
) -> Any | None:
