...
def _with_sample_notice(state: Any, body: Any) -> Any:
    ...
    if (
        ...
        return body
    from src.services.sample_data import notice_already_present, sample_data_notice  # noqa: PLC0415

    if notice_already_present(body):
        return body
    # In the language the agent decided on. The notice is shown ABOVE the answer, so when
    # it stays bilingual after a decision the first thing the reader meets is a paragraph
    # in the other language.
    return sample_data_notice(decided_language(state) or "") + "\n\n---\n\n" + body.lstrip()
...
def _with_disclaimer_impl(
    ...
    *,
    ...
    if not rendered and isinstance(original, str) and not reads_as_prose(original):
        # An EMPTY rendering means the structured payload holds nothing worth saying -- NOT
        # that the raw form should stand in for it. Falling back to `original` here puts
        # back exactly the blob that rendering exists to prevent: measured 2026-09-16 on the
        # another template pod, a reader who asked in romanised Japanese received
        # `{"steps": [], "terminology": {}, "citations": [], "escalation_path": null}` on
        # screen -- status success, str payload, trailer attached, every column green.
        # Dropping it lets the nothing-produced branch below say so in the reader's own
        # language, which is the true statement about that run.
        payload = ""
